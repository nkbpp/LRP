package ru.pfr.xmlparsertempold;

import org.w3c.dom.Document;
import org.w3c.dom.Node;
import org.w3c.dom.NodeList;
import org.xml.sax.SAXException;
import ru.pfr.xmlparsertemp.BadFormatException;
import ru.pfr.xmlparsertemp.Xml10kModel;
...
public class Xml10kRequestParser {
    ...
    public Xml10kParentModel parse(InputStream fis) throws BadFormatException {
        ...
        if(!(xml10kParentModel.getDocumentType().equals("ИНПАСПОРТ") ||
                !(xml10kParentModel.getDocumentType().equals("ПРОЧЕЕ")))
        ){
            xml10kParentModel.setDocumentSeria(findValue(udDocumentBlock, "УТ2:Серия", "УТ:Серия"));
        } else {
            xml10kParentModel.setDocumentSeria("-");
        }
